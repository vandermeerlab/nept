...
def test_power_in_db_simple():
    ...
    assert np.allclose(db_power, np.array([100.0, -3.01029996, 3.01029996]))
...
def test_mean_coherencegram_with_itself():
    ...
    fs = 125
    dt = 1
    timebins, freq, coherencegram = nept.mean_coherencegram(
        perievent1, perievent1, dt, window, fs
    )

    assert np.allclose(coherencegram[0], np.array([1.0, 0.0, 0.0]))
